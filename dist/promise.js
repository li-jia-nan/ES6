...
class MyPromise {
    ...
    // 无论如何都会执行，不会传值给回调函数
        ...
        return this.then(value => MyPromise.resolve(isFunction(onfinally) ? onfinally() : onfinally).then(() => {
            return value;
        }), reason => MyPromise.resolve(isFunction(onfinally) ? onfinally() : onfinally).then(() => {
            throw reason;
        }));
    };
    /**
